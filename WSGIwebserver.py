    ...
    def handler_client(self, client):
        ...
        if not file_name.endswith('.py'):
            ...
            try:
                ...
                env = {}
                self.response_body = m.application(env, self.start_response)
                self.content = self.server_headers + '\r\n' + self.response_body
            except Exception as e:
                print(e)
                header_start = 'HTTP/1.1 404 NOT FIND \r\n'
                headers_1 = 'Content-Type: text/html \r\n'
                response_body = '<h1>sorry not find</h1>'
                self.content = header_start + headers_1 + '\r\n' + response_body
            finally:
                client.send(bytes(self.content, 'utf-8'))

    def start_response(self, status, heardes):
        server_headers = 'HTTP/1.1 ' + status + '\r\n'

        for header in heardes:
            server_headers += '%s: %s\r\n' % (header[0], header[1])
